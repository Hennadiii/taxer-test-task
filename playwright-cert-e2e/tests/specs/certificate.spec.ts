...
test.describe('E2E: Завантаження та перевірка сертифікатів', () => {
  ...
  for (const fileName of certFiles) {
    test(`Файл: ${fileName}`, async ({ page }) => {
      ...
      // Невелика пауза — AngularJS обробляє drop подію асинхронно
      await page.waitForTimeout(1500);

      // Перевіряємо що хоч щось з'явилося у списку
      const itemCount = await page.locator('.list-group-item').count();
      console.log(`📂 Елементів у списку після drop: ${itemCount}`);

      // Якщо список порожній — drop не спрацював, пробуємо ще раз
      if (itemCount === 0) {
        console.warn('⚠️  Повторна спроба drop...');
        await dropFile(page, '.dropbox', filePath);
        await page.waitForTimeout(2000);
      }

      // Чекаємо появи хоча б одного елементу і виводимо його текст
      await page.locator('.list-group-item').first().waitFor({ state: 'attached', timeout: 15_000 });
      const allItems = await page.locator('.list-group-item').allInnerTexts();
      console.log(`📋 Текст елементів у списку: ${JSON.stringify(allItems)}`);
      console.log(`🔍 Шукаємо: ${certInfo.commonName}`);

      // 6. Перевірка списку зліва
      // Використовуємо часткове співпадіння — сайт може відображати скорочений CN
